...
| **Unit Tests (476)** | `mvn test` | ✅ Works on any network (no external deps) |
...
- **476 tests, 0 failures, 0 skipped**
- Run: `mvn test`
- Runtime: ~2-3 seconds
...
| 2026-08-24 | Task 12: Unit tests created (449 unit tests + 23 integration tests = 476 total, including later additions) |
...
| 2026-09-05 | Restricted `GET /leave-requests/{id}` to owner, assigned manager, or admin. Previously any authenticated user could view any request by ID. Now returns 403 if the authenticated user is not the submitter, the assigned manager, or an admin. Added 5 controller tests (owner 200, manager 200, admin 200, other staff 403, non-assigned manager 403) and 2 Postman/Newman edge case tests. |
...
| **Tests** | Controller: 18 tests (4 GET + 10 POST search + 4 commands). Query handler: 17 tests (5 basic + 12 search). Total: 476 tests, all passing. |
